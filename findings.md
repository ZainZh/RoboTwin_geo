...
- Processed-dataset semantic field visualization finding:
  - Processed RoboTwin HDF5 episodes already expose the needed first-version data: `/pointcloud` for the scene cloud and `/object_pointcloud/{A}`, `/object_pointcloud/{B}`, etc. for per-placeholder segmented object point clouds.
  - This makes a dataset-driven visualization path preferable to a snapshot/SAM2-driven path for the first version: it avoids camera-thread, bbox selection, and online mask tracking variables while preserving exactly the point clouds used by DP3 preprocessing.
  - The script can use `object_pointcloud_utils.load_hdf5(...)` and `extract_placeholder_point_cloud(...)` to share the same fallback behavior as `train_objpc.sh`, while using `semantic_feature_utils.load_semantic_model(...)` and `compute_semantic_pointwise_cloud(...)` for `[xyz + semantic embedding]` extraction.
  - For same-class consistency, all selected clouds for a placeholder/checkpoint should be transformed into semantic embeddings first, then colored through one shared PCA projection. The scene overlay can replace colors for scene points nearest to the selected object points, leaving the background original or gray.
  - Processed HDF5 `/pointcloud` is training-sized scene data, commonly 1024 points, and may already reflect postprocess workspace cropping. Full-scene visualization should reconstruct the scene from the processed dataset's `real_zed_sam2_objpc_meta.json -> raw_episode_dir` RGB/depth frames and calibration, without applying workspace bounds. The segmented object clouds can still come from HDF5 `/object_pointcloud/{placeholder}`.
  - Clarified requirement: full-scene visualization should not imply full-scene object extraction. The scene overlay can use raw full RGB/depth, but semantic object embeddings must keep using processed HDF5 object point clouds so the same workspace-constrained mask/object filtering used for inference is preserved.
  - Debug PLY `/media/zheng/Extreme SSD/geo_mani_data/grasp_mug_new/robotwin_objpc/demo_real_zed_sam2_objpc_global/debug/episode0/pointclouds/frame_000000_A.ply` and HDF5 `/object_pointcloud/{A}` frame 0 have matching bounds/centroid, confirming the semantic object source is clean. The observed colored table points came from nearest-neighbor scene recoloring, not a dirty object point cloud.
  - The default visualization overlay should therefore remove full-scene points that match the segmented object point cloud, then insert the semantic-colored object point cloud. Simple append can make colors hard to see, while nearest-neighbor recoloring can color nearby table points when objects touch or hover close to the table.
  - `cut_replace` still uses spatial proximity because processed object point clouds no longer preserve original full-scene point indices. When object points are close to the table, proximity-based deletion can remove nearby tabletop points and create holes. The visualization now exposes `--overlay_cut_z_min` so only scene points above a chosen z height in the current scene/output frame are eligible for deletion.
  - Shared PCA coloring is global within the selected checkpoint, but it visualizes continuous semantic embeddings, not discrete part predictions. Across partial clouds and different frames, PCA colors can still look non-uniform because min/max normalization and outlier points change perceived contrast. Predicted-label visualization should use `sem_logits.argmax` with a fixed canonical-label palette instead.
  - Reference `include/3d_semantic_train/tools/visualize_utonia_universal_field.py` writes `surface_sem_embedding_joint_pca.ply` with `_fit_shared_pca_projection(...)`, which uses `torch.pca_lowrank`. The previous dataset visualization used `np.linalg.svd`; even with the same embedding set, PCA sign choices differed and produced different RGB colors. The visualization PCA now matches the reference joint PCA implementation.
  - A second mismatch with `visualize_utonia_universal_field.py` was the semantic forward input: reference direct point-cloud mode uses fallback/radial normals when the PLY has no normals and random row sampling for query points, while the DP3 helper path used kNN-estimated normals and FPS/original-order query sampling. The visualization script now defaults to `--semantic_forward_mode reference`, while `--semantic_forward_mode dp3` preserves the old DP3 helper behavior.
  - The provided reference output `summary_point_clouds.json` was generated from three point clouds: episode0 frame0, episode0 frame13, and episode1 frame22. Since `surface_sem_embedding_joint_pca.ply` is a joint PCA over every selected sample, a two-frame run cannot exactly match that color space. The default example selection now mirrors those three frames.
  - The reference direct point-cloud inputs are the debug PLYs, whose RGB values are artificial placeholder colors (`{A}` uses `[255,48,48]`) rather than the HDF5 stored object RGB. Utonia uses color as an input feature, so feeding HDF5 RGB changes the embeddings before PCA. The visualization now defaults to `--semantic_input_color_mode debug_placeholder`, with `stored_scaled` and `stored` available for non-reference diagnostics.
- Processed-dataset multi-feature visualization finding:
  - `script/visualize_semantic_field_on_dataset.py` currently assumes one semantic-feature backend and stores the feature tensor in each record as `semantic_cloud`; all PCA/label output paths are tied to that backend.
  - `/home/zheng/github/Utonia/demo/6_pca_object_obj_1.py` uses the public `utonia.load("utonia", repo_id="Pointcept/Utonia")` model, applies `utonia.transform.default(...)`, forwards a collated point dict, upcasts features through two parent-pooling levels, and colors `point.feat` with a PCA helper that mixes the first six PCA components as `0.6 * PC0..2 + 0.4 * PC3..5`.
  - Utonia's default transform applies `CenterShift(apply_z=True)` and grid sampling. For scene overlay, the visualization should restore the center shift before writing PLY coordinates; otherwise the Utonia feature points would be colored correctly but spatially shifted away from the original object in the scene.
  - A DINOv2 per-object point visualization cannot be recovered from processed HDF5 object point clouds alone because those clouds do not preserve source image pixel indices. The practical dataset-driven path is to load the corresponding raw RGB/depth/camera calibration, project each object point into selected camera RGB frames, sample DINOv2 patch features at the projected pixel, and aggregate valid per-camera features per 3D point.
  - To compare feature sources without misleading color alignment, semantic, Utonia, and DINOv2 PCA projections should be fit independently per backend. Shared PCA can still happen across selected frames/objects inside the same backend, but never across different backends.
- Real semantic training/inference feature-input finding:
  - Offline DP3 semantic preprocessing still calls `compute_semantic_pointwise_cloud(...)` without passing a semantic input-color mode or reference forward mode, so it uses the old DP3 defaults: stored object RGB, kNN-estimated normals, and FPS query sampling.
  - Sim/real semantic deployment also calls the same helper without those options, so existing trained checkpoints and online inference do not currently use the debug placeholder colors that matched the reference Utonia visualization.
  - For real processed HDF5 data, `/object_pointcloud/{placeholder}` RGB is stored as real camera color in `0-1` scale, while the validated reference debug PLY path uses artificial placeholder colors in `0-255`. Online SAM2 object-point-cloud extraction also normalizes live RGB to `0-1` before merging.
  - Utonia's `NormalizeColor` transform divides input color by `255`, so feeding already-normalized `0-1` RGB pushes colors to `0-0.0039`. This means current real semantic features are not just appearance-dependent; their color channel is also on a different scale from the reference Utonia direct point-cloud path.
  - If the semantic branch is intended to provide part/geometry semantics rather than appearance cues, the safer real-data path is to make semantic feature extraction explicitly use a deterministic placeholder color policy, and to record that mode in preprocessing meta/checkpoint settings. Existing semantic-hybrid checkpoints should not be mixed with a different semantic feature-input mode at inference.
  - The corrected path now defaults to `semantic_input_color_mode=debug_placeholder` and `semantic_forward_mode=reference`, records both in preprocessing meta, and uses `-semdebugref` in generated semantic zarr/checkpoint suffixes.
  - The copied validation dataset `/home/zheng/Datasets/Training_data/grasp_mug_new/robotwin_objpc/demo_real_zed_sam2_objpc_global/data/episode0.hdf5` stores object RGB in `0-1`; the new semantic input preparation maps `{A}` to `[255,48,48]` and `{B}` to `[48,96,255]` before Utonia, matching the validated debug PLY color scale.
