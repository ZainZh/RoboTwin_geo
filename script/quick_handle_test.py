"""
快速验证把手抓取：只测试指定 model_id，每个 2 seed，打印 plan_success 和 check_success。
"""
import sys, os
sys.path.append(".")
import numpy as np
...
print(f"测试 {len(test_ids)} 个模型, 每个 {seeds_per_model} seed（验证把手抓取）")
...
for mid in test_ids:
    ...
    for seed in range(seeds_per_model):
        try:
            ...
            print(f"  [ERROR] mid={mid} seed={seed}: {e}")
    detail = ("GOOD" if success > 0 else
              "plan_fail" if plan_ok == 0 and stable > 0 else
              "unstable" if stable == 0 else "task_fail")
    line = f"{mid:4d} | {stable:>6} | {plan_ok:>7} | {success:>7} | {detail}"
    print(line)
    sys.stdout.flush()
    # 同时写入文件
    with open("/tmp/handle_quick_progress.txt", "a") as pf:
        pf.write(line + "\n")
    if success > 0:
        good_models.append(mid)

result = f"\n把手抓取可用模型 ({len(good_models)}): {good_models}"
print(result)
with open("/tmp/handle_quick_progress.txt", "a") as pf:
    pf.write(result + "\n")
