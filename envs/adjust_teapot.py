...
class adjust_bottle(Base_Task):
    ...
    def load_actors(self):                                    #往场景里放东西
        self.qpose_tag = np.random.randint(0, 2)    #随机 0或1，决定放左边还是右边
        qposes = [[0.707, 0.0, 0.0, -0.707], [0.707, 0.0, 0.0, 0.707]] #两种初始朝向
        xlims = [[-0.12, -0.08], [0.08, 0.12]]                         #初始x位置范围左边和右边
        ...
        self.bottle = rand_create_actor(
            ...
            modelname="053_teanet",                                   #加载哪个模型（assets/objects/001_bottle/）
            convex=True,                                              #只绕 Z 轴最多旋转 0.4 rad
            rotate_lim=(0, 0, 0.4),
            model_id=self.model_id,                                   #这是干什么的
        )
        self.delay(4)                                                 #等稳定4秒
        self.add_prohibit_area(self.bottle, padding=0.15)             #该区域禁止放别的
    ...
    def play_once(self):
        ...
        # Grasp the bottle with specified arm先移动到物体上方10cm
        ...
        # Place the bottle at target pose (functional point 0)用这个点对齐目标 while keeping gripper closed
        self.move(
            self.place_actor(
                self.bottle,
    ...
    def check_success(self):
        target_hight = 0.9
        bottle_pose = self.bottle.get_functional_point(0)                    #获取这个点的世界坐标
        return ((self.qpose_tag == 0 and bottle_pose[0] < -0.15) or          #物体成功被放到左侧、右侧、举起来
