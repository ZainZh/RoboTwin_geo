...
Add a new DP3 path that keeps the baseline merged object point cloud in `obs.point_cloud` and also injects
`ndf_point_cloud_{placeholder}` as an extra observation. This is intended to test whether NDF pointwise features help
when they are added on top of the baseline object-PCD signal rather than replacing the feature placeholder inside the
main context cloud.

## Problem

The current `ndf_pointwise` path removes NDF-feature placeholders from the main `point_cloud` context and places them
only in `ndf_point_cloud_{placeholder}`. For `hanging_mug`, that means `{A}` mug is removed from the main cloud and only
`{B}` rack remains in `point_cloud`. This changes the observation factorization relative to `objpc` and may discard
localization cues that the baseline policy uses successfully.

The existing global-NDF path is not an adequate substitute because it produces pose-invariant low-dimensional
descriptors and has already shown severe localization regressions during mug approach.
...
Add a distinct preprocess/train/eval/config path whose only semantic difference from `ndf_pointwise` is that
`point_cloud` keeps all placeholders.
...
- If NDF feature extraction itself is too noisy, retaining raw `point_cloud` may only mitigate harm rather than create a
  gain.
...
- The repo can now test whether the NDF issue is primarily caused by replacing raw `{A}` geometry inside the main
  point-cloud branch.
