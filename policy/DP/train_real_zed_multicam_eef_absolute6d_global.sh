...
seed=${4}
gpu_id=${5}
camera_labels=${6:-global,left,right}
...
train_setting="${task_config}-dp-${camera_setting}-eef-absolute6d-global"
exp_name="${task_name}-robot_dp-real_zed_multicam_eef_train"
zarr_path="data/${task_name}-${train_setting}-${expert_data_num}.zarr"

echo -e "\033[33mgpu id (to use): ${gpu_id}\033[0m"
echo -e "\033[33mReal-ZED DP multicam EEF: ${camera_labels}, eef_frame_mode: ${eef_frame_mode}\033[0m"
...
if [ ! -d "./${zarr_path}" ]; then
    ...
    bash process_data_real_zed_multicam.sh \
        "${task_name}" \
        "${task_config}" \
        "${expert_data_num}" \
        "${camera_labels}" \
