...
episode_start="$1"
episode_stop="$2"
if (( episode_start < 45 || episode_start >= episode_stop || episode_stop > 75 )); then
  echo "expected 45 <= start < stop <= 75" >&2
  exit 2
fi
...
log_dir="${artifact_root}/experiment_outputs/fulltask_tagrt_v2/gate2_left60"

mkdir -p "${log_dir}"
export CUDA_VISIBLE_DEVICES="${gpu_id}"
export ROBOTWIN_RAW_DATA_ROOT="${artifact_root}/simulator_data"
export MPLCONFIGDIR="/tmp/tagrt-mpl"
