...
if [ ! -d "${zarr_dir}" ]; then
    bash process_data_semantic_pointwise_hybrid.sh "${task_name}" "${task_config}" "${expert_data_num}" "${semantic_ckpt_A}" "${semantic_ckpt_B}" "${semantic_device}" "${object_placeholders}" "${semantic_point_num}" "${point_cloud_num}" "${output_suffix}"
fi
