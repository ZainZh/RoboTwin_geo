...
from ndf_feature_utils import compute_ndf_pointwise_cloud, load_ndf_model, load_scene_info, summarize_modes
...
from process_data_ndf_pointwise import parse_placeholder_model_args
...
def main():
    parser = argparse.ArgumentParser(
        description="Process RoboTwin episodes into DP3 zarr using actor segmentation for raw object point clouds and point-wise NDF features."
    )
    ...
    parser.add_argument("--ndf_num_points", type=int, default=128)
    parser.add_argument("--output_suffix", type=str, default="-objpc-actorseg-ndf-pointwise-hybrid")
    ...
    if len(placeholders) == 0:
        raise RuntimeError("No object placeholders were found for actorseg NDF hybrid preprocessing.")
    ...
    if len(model_paths) == 0:
        raise RuntimeError("Actorseg NDF hybrid preprocessing requires at least one --ndf_model PLACEHOLDER=CHECKPOINT.")
    ...
    if len(feature_placeholders) == 0:
        raise RuntimeError("None of the requested object placeholders has an NDF model configured.")
    ...
    print(f"Resuming actorseg NDF hybrid preprocessing from episode {start_episode + 1} / {num}")
                ...
                for placeholder in feature_placeholders:
                    episode_feature_arrays[placeholder].append(
                        compute_ndf_pointwise_cloud(
                            model=model_by_placeholder[placeholder],
