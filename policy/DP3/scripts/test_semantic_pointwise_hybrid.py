...
def make_model(*, hybrid: bool) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        use_semantic_pointwise_hybrid=hybrid,
...
class TestSemanticPointwiseHybrid(unittest.TestCase):
    def setUp(self):
        self.fake_semantic_cloud = np.ones((2, 131), dtype=np.float32)

    def test_semantic_pointwise_excludes_feature_placeholder_from_main_context(self):
        with patch.object(
            deploy_policy,
            "get_semantic_utils",
            return_value=(lambda **kwargs: self.fake_semantic_cloud, None),
        ):
    ...
    def test_semantic_pointwise_hybrid_keeps_feature_placeholder_in_main_context(self):
        with patch.object(
            deploy_policy,
            "get_semantic_utils",
            return_value=(lambda **kwargs: self.fake_semantic_cloud, None),
    ...
    def test_hybrid_wrapper_builds_attached_output_suffix_argument(self):
        ...
        forwarded = process_data_semantic_pointwise_hybrid.build_hybrid_argv(argv)
