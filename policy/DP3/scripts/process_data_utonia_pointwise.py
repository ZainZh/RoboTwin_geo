...
def resolve_feature_placeholders(placeholders: list[str], requested: str) -> list[str]:
    requested_list = parse_placeholder_list(requested)
    if len(requested_list) == 0:
        return list(placeholders)
    requested_set = set(requested_list)
    return [placeholder for placeholder in placeholders if placeholder in requested_set]
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Process RoboTwin episodes into DP3 zarr using context object point clouds and raw Utonia point-wise features."
    )
    parser.add_argument("task_name", type=str)
    parser.add_argument("task_config", type=str)
    parser.add_argument("expert_data_num", type=int)
    parser.add_argument("--object_placeholders", type=str, default="")
    parser.add_argument("--utonia_feature_placeholders", type=str, default="")
    parser.add_argument("--utonia_checkpoint", type=str, default="auto")
    parser.add_argument("--utonia_repo_id", type=str, default="Pointcept/Utonia")
    parser.add_argument("--utonia_upcast_levels", type=int, default=0)
    parser.add_argument("--utonia_device", type=str, default="cuda:0")
    parser.add_argument("--utonia_num_points", type=int, default=128)
    parser.add_argument("--output_suffix", type=str, default="-objpc-utonia-pointwise")
...
def main(argv=None):
    ...
    device = torch.device(args.utonia_device if torch.cuda.is_available() else "cpu")
    artifacts = load_utonia_model(
        device=device,
        checkpoint=args.utonia_checkpoint,
        repo_id=args.utonia_repo_id,
        upcast_levels=int(args.utonia_upcast_levels),
    )
    utonia_feat_dim = int(artifacts["feature_dim"])

    feature_placeholders = resolve_feature_placeholders(placeholders, args.utonia_feature_placeholders)
    if len(feature_placeholders) == 0:
        raise RuntimeError("No Utonia feature placeholders were selected.")
    ...
    meta.update(
        {
            ...
            "utonia_num_points": int(args.utonia_num_points),
            "utonia_feat_dim": utonia_feat_dim,
            "keep_feature_placeholders_in_context": bool(args.keep_feature_placeholders_in_context),
        }
    )
    episode_stats = reconcile_episode_stats(meta.get("episodes", []), start_episode=start_episode)

    print(f"Resuming Utonia pointwise preprocessing from episode {start_episode + 1} / {num}")
    ...
    for current_ep in range(start_episode, num):
        ...
        episode_utonia_arrays = {placeholder: [] for placeholder in feature_placeholders}
        ...
        for frame_idx in range(vector_all.shape[0]):
            ...
            if frame_idx != vector_all.shape[0] - 1:
                ...
                for placeholder in feature_placeholders:
                    object_pc = per_placeholder_point_clouds[placeholder]
                    episode_utonia_arrays[placeholder].append(
                        compute_utonia_pointwise_cloud(
                            artifacts=artifacts,
                            object_point_cloud=object_pc,
                            target_num_points=int(args.utonia_num_points),
                        ).astype(np.float32)
                    )
        ...
        for placeholder in feature_placeholders:
            ...
            episode_data[point_cloud_key] = np.asarray(episode_utonia_arrays[placeholder], dtype=np.float32)
        if args.save_placeholder_point_clouds:
            for placeholder in placeholders:
