...
epochs=${EPOCHS:-300}
specs=('field||semantic_point_cloud_A|131' 'partprob|-partprob|semantic_point_cloud_A|5' 'uniformprob|-uniformprob|semantic_point_cloud_A|5' 'shuffledprob|-shuffledprob|semantic_point_cloud_A|5' 'utonia|-utonia|utonia_point_cloud_A|579')
...
while true; do
    ...
    for spec in "${specs[@]}"; do
        ...
        run_name="hanging_mug_${route}_joint14_e${epochs}_seed20260805"
        run_dir="${output_root}/${run_name}"
        weights="${run_dir}/${run_name}.weights.pt"
        manifest="${run_dir}/${run_name}.completion-v2.json"
        if [[ -s "${manifest}" ]]; then (( completed += 1 )); continue; fi
        [[ -s "${weights}" ]] || continue
        args=("${weights}" --manifest "${manifest}" --expected-epoch "${epochs}" --expected-seed 20260805 --expected-task-name "hanging_mug-paper-sim-${route}-joint14-e${epochs}-50" --expected-zarr-path "${data_base}${suffix}.zarr" --expected-precision bf16 --expected-allow-tf32 true --expected-obs-key "${obs_key}" --expected-feature-dim "${feature_dim}" --hydra-dir "${run_dir}/.hydra" --role remote)
        for code_path in "${code_paths[@]}"; do args+=(--code-path "${code_path}"); done
        "${python_bin}" "${finalizer}" "${args[@]}"
        (( completed += 1 ))
    done
    printf '[hanging-seed05-remote-monitor] completed=%s/5 checked_at_utc=%s\n' "${completed}" "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    (( completed == 5 )) && exit 0
    sleep 60
done
