...
beat_root=${BEAT_OUTPUT_ROOT:-${repo_root}/outputs/paper_revision/robotwin_sim_policy_beat_hammer_e300_bf16_v1}
data_marker="${output_root}/hanging_data.remote-ready"
epochs=${EPOCHS:-300}
python_bin=${PYTHON_BIN:-/workspace/venvs/geo-utonia/bin/python}

for marker in "${data_marker}" "${beat_root}/lane_completion/remote_lane0.complete" "${beat_root}/lane_completion/remote_lane1.complete"; do
    until [[ -s "${marker}" ]]; do
        printf '[hanging-remote-lane] lane=%s waiting marker=%s\n' "${lane_id}" "${marker}"
        sleep 120
    done
done

run_job() {
    local seed=$1 route=$2 suffix=$3 feature_dim=$4 obs_key=$5
    env EPOCHS="${epochs}" SEED="${seed}" BATCH_SIZE=256 \
