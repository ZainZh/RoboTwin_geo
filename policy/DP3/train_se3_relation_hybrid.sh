...
python train_dp3.py --config-name=robot_dp3_objpc.yaml \
    ...
    +task.dataset.extra_obs_keys=[se3_relation_token_A_to_B] \
    task.shape_meta.obs.point_cloud.shape=[${point_cloud_num},6] \
    +task.shape_meta.obs.se3_relation_token_A_to_B.shape=[11] \
    +task.shape_meta.obs.se3_relation_token_A_to_B.type=low_dim
