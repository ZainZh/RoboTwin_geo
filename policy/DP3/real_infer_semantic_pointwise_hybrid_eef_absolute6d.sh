...
ckpt_setting="${task_config}-objpc-semantic-pointwise-hybrid-eef-absolute6d-rightbase"
...
output_frame=${12:-right_base}
eef_frame_mode=${13:-${output_frame}}
extra_flags=("${@:14}")
...
python script/real_zed_inference/real_dp3_inference.py \
    ...
    --action_diagnostics_csv outputs/real_zed_inference/action_diag_async_eef.csv \
    "${frame_overrides[@]}" \
    "${extra_flags[@]}"
