...
gate_near_ratio=${27:-0.35}
gate_far_ratio=${28:-0.75}
valid_query_radius=${29:-0.75}
valid_fraction_min=${30:-0.25}
valid_fraction_max=${31:-0.65}

point_cloud_suffix=""
if [ "${point_cloud_num}" != "1024" ]; then
    point_cloud_suffix="-pc${point_cloud_num}"
fi
output_suffix="-objpc-ndf-relation-token-v1${point_cloud_suffix}"
...
if [ ! -d "${zarr_dir}" ]; then
    bash process_data_ndf_relation_token_v1.sh \
        ...
        "${gate_near_ratio}" "${gate_far_ratio}" "${valid_query_radius}" \
        "${valid_fraction_min}" "${valid_fraction_max}"
fi

python scripts/validate_ndf_relation_token_v1_meta.py "${meta_path}"
mapfile -t ndf_meta < <(python -c 'import json,sys; m=json.load(open(sys.argv[1], "r", encoding="utf-8")); print(int(m["ndf_num_points"])); print(int(m.get("ndf_feat_dim", 256))); print(",".join(m.get("feature_placeholders", []))); print(int(m["relation_token_dim"])); print(m["relation_token_gate_near_ratio"]); print(m["relation_token_gate_far_ratio"]); print(m["relation_token_valid_query_radius"]); print(m["relation_token_valid_fraction_min"]); print(m["relation_token_valid_fraction_max"])' "${meta_path}")
ndf_point_num=${ndf_meta[0]}
ndf_feat_dim=${ndf_meta[1]}
ndf_feature_placeholders=${ndf_meta[2]}
relation_token_dim=${ndf_meta[3]}
gate_near_ratio=${ndf_meta[4]}
gate_far_ratio=${ndf_meta[5]}
valid_query_radius=${ndf_meta[6]}
valid_fraction_min=${ndf_meta[7]}
valid_fraction_max=${ndf_meta[8]}
...
exp_name="${task_name}-robot_dp3_ndf_relation_token_v1-train"
