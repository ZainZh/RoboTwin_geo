...
class VNNOccNet(nn.Module):
    def __init__(self,
                 ...
                 return_vector_features=False,
                 vector_feature_dim=0,
                 acts='all',
                 scaling=10.0,
                 decoder_dropout=0.0):
        super().__init__()
        ...
        self.return_vector_features = bool(return_vector_features)
        ...
        self.decoder = DecoderInner(
            ...
            return_vector_features=self.return_vector_features,
            vector_feature_dim=int(vector_feature_dim),
            acts=acts,
    ...
    def forward(self, input):
        ...
        dec_out = self.decoder(query_points, z)
        if self.return_features and self.return_vector_features:
            out_dict['occ'], out_dict['features'], out_dict['vector_features'] = dec_out
        elif self.return_features:
            out_dict['occ'], out_dict['features'] = dec_out
        elif self.return_vector_features:
            out_dict['occ'], out_dict['vector_features'] = dec_out
        else:
            out_dict['occ'] = dec_out
    ...
    def forward_latent(self, z, coords, return_vector_features=False):
        coords = coords * self.scaling 
        dec_out = self.decoder(coords, z)

        features = None
        vector_features = None
        if isinstance(dec_out, tuple):
            if len(dec_out) == 3:
                _, features, vector_features = dec_out
            elif len(dec_out) == 2:
                _, second = dec_out
                if self.return_features:
                    features = second
                elif self.return_vector_features:
                    vector_features = second

        if return_vector_features:
            return features, vector_features
        if features is None:
            raise RuntimeError(
                "forward_latent requires `return_features=True` during model construction."
            )
        return features
...
class DecoderInner(nn.Module):
    ...
    def __init__(self, dim=3, z_dim=128, c_dim=128,
                 hidden_size=128, leaky=False, return_features=False, return_vector_features=False,
                 vector_feature_dim=0, sigmoid=True, acts='all', dropout=0.0):
        super().__init__()
        self.z_dim = z_dim
        self.c_dim = c_dim

        self.acts = acts
        if self.acts not in ['all', 'inp', 'first_rn', 'last', 'inp_first_rn']:
            #self.acts = 'all'
            raise ValueError('Please provide "acts" equal to one of the following: "all", "inp", "first_rn", "last", "inp_first_rn"')
        ...
        self.dropout = nn.Dropout(p=float(dropout)) if dropout and float(dropout) > 0.0 else None
    ...
    def forward(self, p, z, c=None, **kwargs):
        ...
        vec_out = None
        if self.return_vector_features and self.vector_basis is not None and self.fc_vec_alpha is not None:
            z_basis = self.vector_basis(z_vectors)
        ...
        if self.return_features:
            ...
            elif self.acts == 'first_rn':
                acts = torch.cat(acts_first_rn, dim=-1)
            elif self.acts == 'last':
                acts = last_act
