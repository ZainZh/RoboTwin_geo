...
ckpt_setting="${task_config}-eef-absolute6d-rightbase"
...
output_frame=${7:-right_base}
eef_frame_mode=${8:-${output_frame}}
extra_flags=("${@:9}")
...
python script/real_zed_inference/real_dp3_inference.py \
    ...
    --action_diagnostics_csv outputs/real_zed_inference/action_diag_async_baseline_eef.csv \
    "${frame_overrides[@]}" \
    "${extra_flags[@]}"
