...
if [ ! -d "./data/${task_name}-${task_config}-${expert_data_num}-objpc-ndf-pointwise.zarr" ]; then
    bash process_data_ndf_pointwise.sh "${task_name}" "${task_config}" "${expert_data_num}" "${ndf_ckpt_A}" "${ndf_ckpt_B}" "${ndf_device}" "${ndf_dgcnn_/placeholders}" "${object_placeholders}" "${ndf_point_num}"
fi
