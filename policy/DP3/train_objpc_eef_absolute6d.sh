...
output_suffix="-objpc-eef-absolute6d-rightbase${point_cloud_suffix}"
...
if [ ! -d "./data/${task_name}-${task_config}-${expert_data_num}${output_suffix}.zarr" ]; then
    bash process_data_objpc_eef_absolute6d.sh "${task_name}" "${task_config}" "${expert_data_num}" "${object_placeholders}" "${point_cloud_num}" "" "" "" right_base "${output_suffix}"
fi
