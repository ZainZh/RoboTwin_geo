...
if [ ! -d "./data/${task_name}-${task_config}-${expert_data_num}-objpc-ndf-pointwise-hybrid-feat5000.zarr" ]; then
    bash process_data_ndf_pointwise_hybrid_feat5000.sh "${task_name}" "${task_config}" "${expert_data_num}" "${ndf_ckpt_A}" "${ndf_ckpt_B}" "${ndf_device}" "${ndf_dgcnn_placeholders}" "${object_placeholders}" "${ndf_point_num:-5000}"
fi

DEBUG=False
save_ckpt=True
wandb_mode=online
train_setting="${task_config}-objpc-ndf-pointwise-hybrid-feat5000"
exp_name="${task_name}-robot_dp3_ndf_pointwise_hybrid_feat5000-train_ndf"
run_dir="data/outputs/${exp_name}_seed${seed}"
zarr_path="../../../data/${task_name}-${task_config}-${expert_data_num}-objpc-ndf-pointwise-hybrid-feat5000.zarr"
ndf_point_num=${ndf_point_num:-5000}
