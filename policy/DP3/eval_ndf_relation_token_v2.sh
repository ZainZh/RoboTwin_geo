...
projection_seed=${17:-0}
gate_near_ratio=${18:-0.35}
gate_far_ratio=${19:-0.75}
valid_query_radius=${20:-0.75}
valid_fraction_min=${21:-0.25}
valid_fraction_max=${22:-0.65}

point_cloud_suffix=""
if [ "${point_cloud_num}" != "1024" ]; then
    point_cloud_suffix="-pc${point_cloud_num}"
fi
ckpt_setting=${ckpt_setting_base}-objpc-ndf-relation-token-v2${point_cloud_suffix}
...
python script/eval_policy.py --config policy/${policy_name}/deploy_policy.yml \
    ...
    --ndf_relation_token_projection_dim ${projection_dim} \
    --ndf_relation_token_projection_seed ${projection_seed} \
    --ndf_relation_token_gate_geometry true \
