...
max_val_steps=${21:-2}
output_suffix="-objpc-semantic-pointwise-hybrid-eef-absolute6d-rightbase"
...
if [ ! -d "${zarr_dir}" ]; then
    bash process_data_semantic_pointwise_hybrid_eef_absolute6d.sh "${task_name}" "${task_config}" "${expert_data_num}" "${semantic_ckpt_A}" "${semantic_ckpt_B}" "${semantic_device}" "${object_placeholders}" "${semantic_point_num}"
fi
...
save_ckpt=True
wandb_mode=online
train_setting="${task_config}-objpc-semantic-pointwise-hybrid-eef-absolute6d-rightbase"
exp_name="${task_name}-robot_dp3_semantic_pointwise_hybrid_eef_absolute6d-train"
