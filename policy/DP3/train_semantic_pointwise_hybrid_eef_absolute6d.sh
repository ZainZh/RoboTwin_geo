...
output_suffix="-objpc-semantic-pointwise-hybrid-eef-absolute6d-rightbase${point_cloud_suffix}"
...
if [ ! -d "${zarr_dir}" ]; then
    bash process_data_semantic_pointwise_hybrid_eef_absolute6d.sh "${task_name}" "${task_config}" "${expert_data_num}" "${semantic_ckpt_A}" "${semantic_ckpt_B}" "${semantic_device}" "${object_placeholders}" "${semantic_point_num}" "" "" "" right_base "${point_cloud_num}" "${output_suffix}"
fi
