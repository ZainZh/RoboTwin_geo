...
output_suffix=${11:--objpc-ndf-relation-token-v2}
ndf_feat_dim=${12:-256}
projection_dim=${13:-16}
projection_seed=${14:-0}
gate_near_ratio=${15:-0.35}
gate_far_ratio=${16:-0.75}
valid_query_radius=${17:-0.75}
valid_fraction_min=${18:-0.25}
valid_fraction_max=${19:-0.65}
...
python scripts/process_data_ndf_relation_token_v2.py \
    "${task_name}" \
    "${task_config}" \
    "${expert_data_num}" \
    ...
    --relation_token_projection_dim "${projection_dim}" \
    --relation_token_projection_seed "${projection_seed}" \
